...
create or replace temp view sales_parsed_csv  (order_id LONG, email STRING, transactions_timestamp LONG, total_item_quantity INTEGER, purchase_revenue_in_usd DOUBLE, unique_items INTEGER, items STRING) 
using csv 
options (path="${da.paths.datasets}/ecommerce/raw/sales-csv",header="true",delimiter"|")

-- COMMAND ----------

create table sales_parsed_csv_delta as
select * from sales_parsed_csv
...
create or replace table purchase_ids (
  id string,
  transaction_timestamp string,
  id_gen string generated always as (replace (id, (substr(id,1,4)),'xxxx') )

)

-- COMMAND ----------

select replace (a,'D','P') from (
select(substr('DEEPAK',1,4)) as a ) as b 
...
alter table purchase_ids add constraint id_check check (len(id)>=4)
...
DESCRIBE EXTENDED purchase_ids
...
select cast (user_first_touch_timestamp/1e6 as timestamp) from parquet.`${da.paths.datasets}/ecommerce/raw/users-historical/`
